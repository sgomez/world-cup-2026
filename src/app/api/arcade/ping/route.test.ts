...
describe("POST /api/arcade/ping", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const req = new Request("http://localhost/api/arcade/ping", {
      method: "POST",
  ...
  it("returns 200 when heartbeat is recorded successfully", async () => {
    mockGetSession.mockResolvedValue({
      user: { id: "user-alice", role: "user" },
      session: { id: "session-1" },
    } as any);

    mockRecordHeartbeat.mockReturnValue(okAsync(undefined));
  ...
  it("returns 404 when run is not found", async () => {
    ...
    mockRecordHeartbeat.mockReturnValue(errAsync(domainError("RUN_NOT_FOUND")));
  ...
  it("returns 409 when run is not in_progress", async () => {
    ...
    mockRecordHeartbeat.mockReturnValue(
