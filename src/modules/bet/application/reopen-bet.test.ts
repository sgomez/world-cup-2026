...
import { reopenBet } from "./reopen-bet";
...
describe("reopenBet use-case", () => {
  ...
  it("distinguishes NOT_FOUND from FORBIDDEN", async () => {
    const repo = new InMemoryBetRepository([bet()]);
    const missing = await reopenBet(repo, {
      betId: "missing",
      userId: "someone-else",
      window: WINDOW,
      now: BEFORE,
  ...
  it("reopens the bet and persists status=draft", async () => {
    const repo = new InMemoryBetRepository([bet()]);
    const result = await reopenBet(repo, {
      betId: "bet-1",
      userId: OWNER,
