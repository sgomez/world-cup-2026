...
### Match Link
An optional URL stored on a **LiveResult** as internal metadata. It is write-only on the API: settable at creation time (POST) and updatable or clearable via PATCH, but never returned in any API response and ignored by PUT. It carries no scoring or display logic — it is opaque data owned by the writers of the LiveResult.
