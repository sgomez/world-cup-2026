...
### Share Link
A public, unauthenticated URL — keyed by the **Community** **slug**, not a secret token — that any **Community Member** may share to promote their Community's **Leaderboard** on external platforms. It is **not** an **Invite Link**: it grants no join capability and exposes no prediction content. Visiting it as a human redirects to the authenticated Leaderboard; visiting it as a link-preview crawler yields an OpenGraph card showing the Community name and the best three and worst three Bets by **label** and **Score** — never the Bet owner's name. It is *semi-public* by design: its only protection is that the slug derives from the Community name, which is known only to members and invitees. Disabled for **Imported Communities**, whose labels carry obfuscated real-world names (see **Label Obfuscation**).
...
### Rank History
The chronological sequence of a closed **Bet**'s rank positions on a **Leaderboard**, calculated after each finished **Match Number**. It is derived on-read from the sequence of finished **LiveResults**, starting from a baseline position of 1 before the first match (Match 0) and ending with the current **Provisional Result** if any matches are `live`. On the user interface, it displays the progression of the logged-in **User**'s Bets alongside the Top 10 Bets in that **Community** to prevent visual clutter, with the last provisional segment styled distinctly.
...
### LiveResult
The real-world score of a single match — the **single source of match facts** for the whole tournament, from which the entire **Result** is derived. A LiveResult records goals for each side, an optional penalty-shootout score (knockout only), a **status** of `upcoming` (not started), `live` (in play, scoreline may still change), or `finished` (full time), and an optional **Match Link**. A match has *no* LiveResult or is set to `upcoming` when not started. Each match is keyed by its **Match Number**. LiveResults have **two writers** using three API operations: POST (create — fails with `ALREADY_EXISTS` if the match number is already present), PUT (create-or-replace, ignores the **Match Link**), and PATCH (partial update — all fields optional, merged onto the existing record). Both the external bot and Admins use the token-authenticated live API; the bot cannot revert `finished` matches to `live` due to a latch, while Admins may transition matches freely. The finishing order shown in the standings is re-derived from LiveResults (including `live` ones, so the table moves provisionally during a match); **Advanced** status and knockout winners use only `finished` LiveResults. LiveResults are surfaced to all users on the calendar and standings; a team in a `live` match is shown with a flashing live marker.

### Match Link
An optional URL stored on a **LiveResult** as internal metadata. It is write-only on the API: settable at creation time (POST) and updatable or clearable via PATCH, but never returned in any API response and ignored by PUT. It carries no scoring or display logic. It is the per-match address a real **Live Feed** reads to fetch that match's current facts; a match with no Match Link cannot be polled by a real Feed and is skipped that **Tick**.
...
### Live Feed
The external source of in-play match facts the **Live Feed Poller** reads — one snapshot per `live` match: the current scoreline and whether the match has **finished**. It is the authority on *when* a match ends (the Poller never decides this from a clock of its own). Which Feed runs is chosen by configuration, defaulting (fail-safe) to a mock: the mock echoes the stored scoreline (so it never resets an Admin correction) and reports `finished` once a match's nominal duration has elapsed since **Kickoff**. A real Feed reads each match's **Match Link** to obtain its current scoreline and finished state, echoes any Admin-entered penalties untouched, and treats a match that is not yet visibly under way as still `live` — so a late real-world kickoff is never mistaken for full time. The provider is deliberately **source-agnostic**: the platform names no specific data source (see ADR 0030), and any future Feed is a new sibling behind the same port without changing the Poller.
