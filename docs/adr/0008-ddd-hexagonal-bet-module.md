...
**Reads consult the window too.** The same `BettingWindow` answers read questions, not just writes. Member visibility (CONTEXT.md *Community Member*) is a pure query on the aggregate — `Bet.peerVisibility(window, now)` returning `"hidden" | "summary" | "full"` (full past the deadline; label+owner+signature for a `closed` Bet before; hidden for a `draft` before). The rule depends only on the Bet's own status plus the injected window, so it lives on the aggregate, not on `Community` (which would couple two aggregates) nor on `BettingWindow` (a pure time gate). The peer read path is a dedicated application service `peerSummariesByOwners` that drops `"hidden"` and projects the rest, kept separate from `summariesByOwners` so admin reads — which see every Bet — stay unfiltered.
